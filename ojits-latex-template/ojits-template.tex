...
\author{Marcos Batista Figueredo\IEEEauthorrefmark{1}, \IEEEmembership{Member, IEEE},
...
\input{generated/tab_stage_by_model}

\input{generated/fig_confusion_stage1}
...
\input{generated/tab_flat_vs_hier}
...
\input{generated/tab_real_synthetic}
...
\input{generated/tab_split_ablation}
...
\subsection{Findings Relative to the Stated Hypothesis}
The problem formulation stated that the hierarchy changes family-level
decisions, whereas subtype behavior depends on the routing produced by
Stage 1, and it named four aspects to examine. Each is taken in turn.

At the family level, Stage 1 exceeds the flat baseline in both releases, with
macro-F1 of 0.5864 against 0.4962 in the original release and 0.8796 against
0.8726 in the extended release. The margin is wide in the first case and
narrow in the second.

At the subtype level under oracle routing, Stage 2 exceeds the flat
classifier, reaching 0.4233 against 0.3757 and 0.4289 against 0.4130.

Under predicted routing the relation reverses and the flat classifier reaches
0.3479 against 0.3236 and 0.3903 against 0.3753. Scored end-to-end over every
truly PQ/Hybrid record, the flat classifier again leads in the original
release, 0.3757 against 0.3695, whereas in the extended release the two values
lie within 0.002 of each other and one seed reverses their order, so no
ordering is claimed for that scope in that release.

The patterns persist unevenly. The reversal under predicted routing holds in
both releases and in all three seeds, and its paired bootstrap interval
excludes zero. Across trace types the picture is not uniform, since records
with real traces carry the higher Stage 1 value in the original release and
the lower one in the extended release, which is an observed association rather
than a property of the architectures compared here.

The hypothesis is supported in substance. The two architectures do differ at
the family level, and subtype behavior does depend on the routing, to the
point of reversing which architecture leads. What the hypothesis did not
anticipate is that the size of that reversal, and in one release its
direction, depend on which rows the comparison scores. Separately from the
hypothesis, the splitting-rule ablation bounds what these numbers would have
been under a random split, where the same models reach macro-F1 above 0.98.

\subsection{Design Considerations for Vehicular Deployment}
...
A single-pass flat classifier has no forwarding decision that can fail, so it cannot lose a record to misrouting. The cascade can. In the original release a substantial share of the records reaching Stage 2 do not belong to its label space at all, while some genuine PQC-related attacks never arrive. For a safety-relevant detector this is a qualitatively different failure mode from an ordinary misclassification, because the record leaves the analysis path entirely.

If a hierarchical architecture is chosen, the family stage is the natural place to partition work, with a lightweight Stage 1 triaging traffic locally on the OBU or RSU and suspected PQC-related payloads offloaded to MEC or Traffic Management Center infrastructure for subtype classification. These results suggest gating that offload on calibrated confidence rather than on the arg-max family decision, since the unconditional forwarding of the arg-max decision is what produces the reversal reported here. Confidence-gated routing was not evaluated in this study and remains an open direction.
...
The protocol controls exact-duplicate leakage, which is the dominant form in this dataset, but it does not establish that the data are leakage-free in every sense. The limitations below bound what the results support, and each states what would be needed to remove it.

\begin{itemize}
    \item \textbf{Dataset-based benchmarking versus physical testbeds.} The analysis relies on the PQ-V2X dataset, a tabular representation of mobility, channel, and post-quantum cryptographic parameters across real and synthetic traces. It is an offline representation, so nothing reported here is evidence about behavior on vehicular hardware. Validating flat and hierarchical detectors on hardware-in-the-loop (HIL) testbeds, with end-to-end processing latency, memory, and energy measured on embedded automotive microcontrollers and OBUs, is left to future work.
    ...
    \item \textbf{Duplicate grouping versus trajectory tracking.} To prevent optimistic performance estimates caused by exact-duplicate leakage, records are grouped by a deterministic feature hash. This controls exact repetition of feature patterns across partitions. Consecutive messages generated during a single vehicle trip may nonetheless exhibit slight feature drift (e.g., gradually changing GPS coordinates or timestamps), forming near-duplicates that the hash does not group. Datasets carrying standardized trajectory identifiers would allow grouping at the trip level, which is stricter than what is possible here.
...
The authors thank the Coordena\c{c}\~{a}o de Aperfei\c{c}oamento de Pessoal
de N\'{i}vel Superior (CAPES) and the Universidade do Estado da Bahia (UNEB)
for the support that made this work possible.
...
(Member, IEEE) holds the M.Sc. and Ph.D. degrees in Computational Modeling and Industrial
...
is a dental surgeon and an M.Sc. student in Biosystems Modeling and Simulation at Universidade do Estado da Bahia. She holds a degree in Dentistry from FATEC-BA and served as Project Director of the Academic League of Oral and Maxillofacial Surgery and Traumatology (2024--2025). Her research focuses on regenerative medicine, digital health, and computational modeling.
\end{IEEEbiographynophoto}
...
Modeling and Industrial Technology at Universidade SENAI-CIMATEC. His
...
from Universidade SENAI-CIMATEC. He is a professor at Universidade
...
holds degrees in Information Systems (Est\'{a}cio FIB, 2009) and Mathematics (Famart, 2022), with specializations in Automation, Control and Robotics (Universidade SENAI-CIMATEC, 2016) and Educational Technologies (UNIAENE, 2025). She is currently an M.Sc. student in Biosystems Modeling and Simulation at Universidade do Estado da Bahia and a vocational education teacher in Bahia. Has professional experience in systems development, programming, databases, and educational robotics. Her research interests include artificial intelligence, mathematical modeling, data science, fuzzy systems, and energy recovery.
\end{IEEEbiographynophoto}


\begin{IEEEbiographynophoto}{Aline de Lurdes Zuliani Lunkes}
received the degree in Mathematics from Universidade Federal de Santa Maria, Brazil, in 2015, the M.Sc. degree in Computational Modeling from Universidade Federal do Rio Grande, Brazil, in 2018, and the Ph.D. degree in Computational Modeling from the Laborat\'{o}rio Nacional de Computa\c{c}\~{a}o Cient\'{i}fica, Brazil, in 2023. She is currently a professor at Universidade Federal do Pampa. Her research interests include computational modeling, artificial intelligence, inverse problems, numerical methods, and scientific computing.
\end{IEEEbiographynophoto}
...
Universidade do Estado da Bahia and at Universidade SENAI-CIMATEC. His
